...
def StaticEstimation(input):
    ...
    if black <= 2:
        ...
        cal = (1000 * (white - black)) + 430*(white+black)
        return cal
...
def MaxMin(input, depth):
    ...
    if depth == 0:
        ...
        depth -= 1
        x = -999999
        midGame = generateMovesMidgameEndgame(input)
        for i in midGame:
            minBaord = MinMax(i, depth)
            if x < StaticEstimation(minBaord):
                x = StaticEstimation(minBaord)
